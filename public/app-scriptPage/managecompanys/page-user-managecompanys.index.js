$(function () {
  ...
  $(document).on('click', '.deleteCompanyBtn', function () {
    ...
    const companyId = $(this).attr('companyId');
    const companyUsername = $(this).attr('companyUsername');
    ...
    Swal.fire({
      ...
      <p style="font-size: 15px; text-align: left;" class="mb-0">
        ...
        <code style="font-size: 15px;">${companyUsername}</code>
      </p>`,
      ...
      didOpen: () => {
        ...
        input.addEventListener('input', () => {
          confirmButton.disabled = input.value?.trim() !== companyUsername?.trim();
        });
      ...
      if (result.isConfirmed) {
        ...
        let settings = {
          "url": `${rootPath}/user/api/managecompanys/delete/${companyId}`,
          "method": "DELETE",
          "headers": {
        ...
        $.ajax(settings)
          .done(async function (response) {
            ...
            // ลบรายการในตาราง
            companysTable.row(rowToDelete).remove().draw(false);
  ...
  $(document).on('click', '.twofactorOffCompanyBtn', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let settings = {
          "url": `${rootPath}/user/api/managecompanys/twofactorclose/${companyId}`,
          "method": "DELETE",
          "headers": {
            "Content-Type": "application/json"
          },
  ...
  $('.validate-form').on('submit', function (event) {
    ...
    if (Form.valid() == true && newPassword) {
      ...
      let settings = {
        "url": `${rootPath}/user/api/managecompanys/password/update`,
        "method": "PUT",
        "headers": {
  ...
  let companysTable = $('#companysTable').DataTable({
    // dom: 'lBfrtip',
